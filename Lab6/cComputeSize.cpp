...
void cComputeSize::Visit(cVarDeclNode * start)
{
	...
	start->setSize(temp_s);
	
	if(temp_s >= 4)
	{
	    temp_o = m_offset + Align(m_offset); 
	    start->setOffset(temp_o);
	    m_offset = temp_s + temp_o;
	    
	}
	else
	{
	    start->setOffset(m_offset);
	    m_offset += temp_s;
	}
...
void cComputeSize::Visit(cDeclsNode * start)
{
	...
	{
	     VisitParams(start);
	     dynamic_cast<cParamsNode*> (start)->setSize(m_offset - temp_o);
	}*/
//	else
//	{
	     VisitAllChildren(start);
...
void cComputeSize::Visit(cVarExprNode * start)
{
	...
	start->setOffset(m_offset);
	//start->GetType()->Sizeof()
	start->setSize(start->GetType()->Sizeof());
	temp_o = start->getSize();
	m_offset = temp_o;
...
{
    m_offset += sym->GetDecl()->getOffset();
}
